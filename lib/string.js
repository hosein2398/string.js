...
//here goes my codes ***********************************************************************
      addhttp: function () {
            ...
            if (!(s.includes(".com"))) {
                http = http + s + com;
            } else if( !(s.includes("www.")) ) {
                http = http + ww + s;
            }else if ( !(s.includes(".com"))  &&  !(s.includes("www.")) )
                {
                 http = http + ww + s + com;
                }
            else {
      ...
      isEqual: function (realq) {
            var s = this.s;
            var testarr = s.split(" ");
            var realarr = realq.split(" ");
            for (var i = 0; i < realarr.length; i++) {
                if (testarr[i] != realarr[i]) {
                    return false;
                }
            }
            return true;
        } ,
       addAfter: function (afterwhat, whatadd) {
            var test = this.s;
            var testarr = test.split(" ");
            var then;
            for (var i = 0; i < testarr.length; i++) {
                if (testarr[i] == afterwhat) {
                    testarr[i] += whatadd;
                }
            }
            var end = testarr.join(" ");
            return end;
        } , 
      
      addBefore: function (beforewhat, whatadd) {
                var test = this.s;
                var testarr = test.split(" ");
                var then;
                for (var i = 0; i < testarr.length; i++) {
                    if (testarr[i] == beforewhat) {
                        testarr[i] = whatadd + testarr[i];
                    }
                }
                var end = testarr.join(" ");
                return end;
            },
       wrap : function (first , sec ) {
            
              var test = first + this.s + sec;  
             return test;  
        } , 
